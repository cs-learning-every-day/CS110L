...
impl Grid {
    ...
    pub(self) fn convert_to_idx(&self, row: usize, col: usize) -> usize {
        return row * self.num_cols + col;
    }

    pub(self) fn check_index(&self, row: usize, col: usize) -> bool {
    ...
    pub fn get(&self, row: usize, col: usize) -> Option<usize> {
        ...
        let idx = self.convert_to_idx(row, col);
        let res = self.elems.get(idx).unwrap();
        return Some(*res);
    }
    ...
    pub fn set(&mut self, row: usize, col: usize, val: usize) -> Result<(), &'static str> {
        ...
        let idx = self.convert_to_idx(row, col);
        *self.elems.get_mut(idx).unwrap() = val;
        return Ok(());
    }
