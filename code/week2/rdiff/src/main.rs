...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let m = seq1.len();
    let n = seq2.len();
    let mut res: Grid = Grid::new(m + 1, n + 1);
    for i in 0..m + 1 {
        res.set(i, 0, 0).unwrap();
    }

    for i in 0..n + 1 {
        res.set(0, i, 0).unwrap();
    }

    for i in 0..m {
        for j in 0..n {
            if seq1.get(i) == seq2.get(j) {
                let val = res.get(i, j).unwrap() + 1;
                res.set(i + 1, j + 1, val).unwrap();
            } else {
                let val = cmp::max(res.get(i, j + 1).unwrap(), res.get(i + 1, j).unwrap());
                res.set(i + 1, j + 1, val).unwrap();
            }
        }
    }
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("invalid filename1");
    let lines2 = read_file_lines(filename2).expect("invalid filename2");
